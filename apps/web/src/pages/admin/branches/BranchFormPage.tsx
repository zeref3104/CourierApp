...
export default function BranchFormPage() {
  ...
  useEffect(() => {
    ...
    branchService.findById(id)
      ...
      .catch(() => alert(t('branches.loadError')))
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">{isEdit ? t('branches.editTitle') : t('branches.new')}</h1>
      <Card>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">{t('branches.manager')}</label>
            <select
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button type="button" variant="secondary" onClick={() => navigate('/branches')}>{t('common.cancel')}</Button>
          </div>
        </form>
      </Card>
    </div>
